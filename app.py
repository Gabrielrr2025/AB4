...
st.caption("Correção de números com múltiplos pontos (ex.: 3.491.40 → 3491,40). Busca, paginação, seleção persistente.")
...
def br_to_float(txt: str):
    ...
    if not t:
        return None
    # Caso BR clássico: tem vírgula
    if "," in t:
        try:
            return float(t.replace(".", "").replace(",", "."))
        except Exception:
            return None
    # Caso PDF: múltiplos pontos (ex.: '3.491.40' => último ponto é decimal)
    if t.count(".") >= 2:
        parts = t.split(".")
...
def dec_places(tok: str) -> int:
    if not tok:
        return 0
    # normaliza ponto-vírgula só para contar casas
    s = tok.replace(".", ",")
    if "," in s:
...
def parse_lince_lines_to_list(text: str):
    ...
    cleaned = []
    for ln in lines:
        ln = re.sub(r"\b\d{8,13}\b\s*$", "", ln).strip()  # EAN no fim
        ln = re.sub(r"\b\d{4,8}\b\s*$", "", ln).strip()   # código no fim
        cleaned.append(ln)
    ...
    for ln in cleaned:
        ...
        if i_qtd is None or i_val is None:
            # fallback: últimos dois números do tail
...
if uploaded:
    ...
    setor = st.text_input("Setor", value=guess_setor(all_text, uploaded.name))

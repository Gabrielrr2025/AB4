...
def parse_lince_lines_to_list(text: str):
    ...
    Formato específico: [classif] [codigo_barras] [codigo] [NOME] [custo] [qtd] [valor] [valor_acum] [%] [%_acum] [preco_venda]
    ...
    lixo_keywords = [
        ...
        "Qtde", "Valor", "Vl. Acum", "Acum.", "Produto", "Venda", "R$"
    ]
    ...
    for ln in lines:
        ...
        # Pula linha que é só número de setor (ex: "1 LANCHONETE")
...
, ln):
        ...
        tokens = ln.split()
        if len(tokens) < 8:  # precisa ter pelo menos os campos básicos
            continue
        
        # Verifica se começa com classificação + código de barras + código interno
        if not (re.match(r'^\d+
...
, tokens[2])):
        ...
        # Extrai campos fixos
        classificacao = tokens[0]
        codigo_barras = tokens[1] 
        codigo_interno = tokens[2]
        
        # Encontra onde termina o nome (procura sequência de números)
        nome_tokens = []
        valor_start_idx = 3
        
        for i in range(3, len(tokens)):
            if is_num_token(tokens[i]):
                # Verifica se os próximos tokens também são números
                if i + 2 < len(tokens) and is_num_token(tokens[i+1]) and is_num_token(tokens[i+2]):
                    valor_start_idx = i
                    break
            nome_tokens.append(tokens[i])
        
        if not nome_tokens or valor_start_idx >= len(tokens) - 2:
            continue
            
        nome = " ".join(nome_tokens).strip()
        
        # Extrai valores numéricos na ordem correta
        valores_tokens = tokens[valor_start_idx:]
        if len(valores_tokens) < 3:
            continue
            
        # Ordem esperada: [custo] [quantidade] [valor] [valor_acum] [%] [%_acum] [preco_venda]
        try:
            custo = br_to_float(valores_tokens[0])
            quantidade = br_to_float(valores_tokens[1]) 
            valor = br_to_float(valores_tokens[2])  # Este é o valor correto!
            ...
            if not nome or len(nome) < 3:
                continue
            if not re.search(r"[A-Za-zÀ-ÖØ-öø-ÿ]{2,}", nome):
                continue
    ...
    # Remove duplicatas e agrega por nome
    agg = {}
